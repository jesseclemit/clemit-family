  ...
  return json({ ok: true });
      ...
      if (p === "/api/notes" && req.method === "GET") {
        ...
        for (const n of all) {
          ...
          out.push({ id: n.id, owner_name: n.owner_name || "", body: n.body || "", created_at: n.created_at, updated_at: n.updated_at, mine: mine, kingView: kingView, shares: mine ? shares.filter(function(x){ return !x.until || x.until > now; }) : [] });
        }
...
function notesOpenPanel(){window.__notesOpen=true;window.__fcat=null;window.__notes=null;render();notesLoad();}
...
function notesSave(){var t=document.getElementById('nnb');if(!t||!t.value.trim())return;notesPost('/api/note',{body:t.value}).then(function(){t.value='';try{localStorage.removeItem('nndraft');}catch(e){}notesLoad();});}
