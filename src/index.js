...
if(order.length){h+='<div style="margin-top:10px">';order.forEach(function(mname,gi){var items=groups[mname];var toBuy=items.filter(function(x){return !x.checked;}).length;h+='<details open class="gmeal" style="margin:8px 0;border:1px solid rgba(0,229,255,.28);border-radius:10px;padding:6px 11px;background:rgba(0,229,255,.04)">';h+='<summary style="cursor:pointer;font-weight:bold"><span class="gcaret">&#9656;</span>🍽 '+esc(mname)+' <span class="badge">'+toBuy+' to buy</span> <button class="mini" onclick="event.preventDefault();saveRecipeI('+gi+')">save recipe</button> <button class="mini" onclick="event.preventDefault();mealDelI('+gi+')">x all</button></summary>';h+='<div class="gnest"><ul>';items.forEach(function(it){h+=gRow(it,true);});h+='</ul></div></details>';});h+='</div>';}
