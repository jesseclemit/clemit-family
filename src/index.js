...
function booksReel(k){var list=(S.media||[]).filter(function(x){return x.kind===k;});var kl=(k==='audiobook')?'audiobooks':'books';if(!list.length){return '<div class="card"><h2 style="font-size:1rem;margin-bottom:4px">No '+kl+' yet</h2><div class="sub">Add one under the + Add shelf - it appears here on the shelf.</div></div>';}window.__cflist=list;if(typeof window.__cfi!=='number'||window.__cfi>=list.length)window.__cfi=0;var h='<div class="cf"><button class="cfnav cfprev" onclick="cfPrev()">&#8249;</button><div class="cfstage" id="cfstage">';list.forEach(function(m,i){var t=m.title||m.caption||'Untitled';h+='<div class="cfc'+(m.img?' hascover':'')+'" onclick="cfGo('+i+')"><div class="cfflip"><div class="cffront">'+cfPosterFor({t:t,img:m.img})+'<div class="cfinfo"><div class="cfd-title2">'+esc(t)+'</div><div class="cfd-meta">'+esc(m.people||'')+'</div></div></div></div></div>';});h+='</div><button class="cfnav cfnext" onclick="cfNext()">&#8250;</button></div>';setTimeout(cfPaint,30);return h;}
