...
if (p === "/api/king/veto" && req.method === "POST") {
  ...
  return json({ ok: true });
      ...
      if (p === "/api/reminders" && req.method === "POST") {
        ...
        await env.DB.prepare("INSERT INTO reminders (owner_email,owner_name,title,body,due_at,channels,to_email,to_phone,repeat_kind,status,created_at) VALUES (?,?,?,?,?,?,?,?,?, 'pending', ?)").bind((me.email || "").toLowerCase(), me.name || "", String(b.title || "").slice(0, 200), String(b.body || "").slice(0, 2000), due, chans, b.to_email || null, b.to_phone || null, String(b.repeat_kind || ""), Date.now()).run();
        return json({ ok: true });
      }
