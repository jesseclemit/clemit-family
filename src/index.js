...
if (p === "/api/king/veto" && req.method === "POST") {
  ...
  return json({ ok: true });
      ...
      if (p === "/api/reminders" && req.method === "POST") {
        ...
        await env.DB.prepare("INSERT INTO reminders (owner_email,owner_name,title,body,due_at,channels,to_email,to_phone,repeat_kind,status,created_at) VALUES (?,?,?,?,?,?,?,?,?, 'pending', ?)").bind((me.email || "").toLowerCase(), me.name || "", String(b.title || "").slice(0, 200), String(b.body || "").slice(0, 2000), due, chans, b.to_email || me.email || null, b.to_phone || null, String(b.repeat_kind || ""), Date.now()).run();
        return json({ ok: true });
      }
...
function noteQuickSend(){var ta=document.getElementById('nqText');if(!ta)return;var v=ta.value.trim();if(!v)return;var type=__nqType,acts=__nqActs;if(typeof audit==='function')audit('post',v.slice(0,80),'Note/'+type);if(type==='shopping'){post('/api/grocery',{name:v,freq:'once'});nqReset();nqToast('Added to Shopping List ✓');return;}var body=(type==='idea'?'💡 ':'')+v;notesPost('/api/note',{body:body,important:acts.urgent?1:0}).then(function(r){var id=r&&r.id;var jobs=[];if(acts.king&&id&&window.S&&S.king)jobs.push(notesPost('/api/note/share',{id:id,email:S.king,hours:0}));if(acts.reminder){var due=window.__remExact?window.__remExact:(Date.now()+remStepMs(REMSTEPS[window.__remIdx==null?1:window.__remIdx]));jobs.push(notesPost('/api/reminders',{title:v.slice(0,60),body:v,due_at:due,channels:['inapp','email'],repeat_kind:''}));}Promise.all(jobs).then(function(){nqReset();nqToast('Saved to Personal Notes ✓');if(typeof notesLoad==='function')notesLoad();if(typeof remLoad==='function')remLoad();});});}
